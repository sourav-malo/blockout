...
function change_pit(el, canvas, ctx) {
  var dimensions = el.toLowerCase().split('x');
...
function refresh_score() {
  $('#score').text(parseInt(STATE.score));
}
...
$(document).ready(function () {
  ...
  $("#pieces").change(function (e) {
    e.preventDefault();
    var selectedPieces = $(this).children("option:selected");
    change_set(selectedPieces);

    $("#pieces .button").removeClass("on");
    $("#pieces .button").removeAttr('selected');

    selectedPieces.addClass("on");
    selectedPieces.attr('selected', 'selected');
  ...
  $("#rotSpeed .button").each(function() {
      if ($(this).val().toLowerCase() == rotateSpeed) $(this).addClass("on");
  });
  $("#rotSpeed").change(function (e) { 
    e.preventDefault();
    var selectedRotSpeed = $(this).children("option:selected");
    setRotateSpeed(selectedRotSpeed.val().toLowerCase());
    
    $("#rotSpeed .button").removeClass("on");
    $("#rotSpeed .button").removeAttr('selected');

    selectedRotSpeed.addClass("on");
    selectedRotSpeed.attr('selected', 'selected');
  ...
  $('#pit .button').each(function () {
    if ($(this).val().toLowerCase() == pit_string) $(this).addClass('on');
  });

  $("#pit").change(function (e) {
    e.preventDefault();

    var selectedPit = $(this).children("option:selected");
    change_pit(selectedPit.val(), canvas, ctx);

    $("#pit .button").removeClass("on");
    $("#pit .button").removeAttr('selected');

    selectedPit.addClass("on");
    selectedPit.attr('selected', 'selected');

  });

  $('#speedSelect .button').each(function () {
    if ($(this).val().toLowerCase() == SPEED) $(this).addClass('on');
  });

  $("#speedSelect").change(function (e) { 
    e.preventDefault();

    var selectedSpeed = $(this).children("option:selected");
    change_speed(selectedSpeed);
    $("#speedSelect .button").removeClass("on");
    $("#speedSelect .button").removeAttr('selected');

    selectedSpeed.addClass("on");
    selectedSpeed.attr('selected', 'selected');
